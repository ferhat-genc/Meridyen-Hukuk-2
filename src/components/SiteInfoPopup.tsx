...
const reasons = [
  {
    icon: "🔍",
    text: "Google'da ve ChatGPT gibi yapay zeka aramalarında görünür olursunuz.",
  },
  {
    icon: "💬",
    text: "Danışanlar size WhatsApp üzerinden tek tıkla ulaşır — aramayla ya da form doldurmayla uğraşmaz.",
  },
  {
    icon: "🗂️",
    text: "Uzmanlık alanları ve ekip bilgileriyle müvekkili ilk görüşmeden önce ikna edersiniz.",
  },
  {
    icon: "⭐",
    text: "Kurumsal bir site, rakip bürolarınızın önüne geçmenizi sağlar.",
  },
  {
    icon: "⚡",
    text: "Bir kez kurulur, içerik ve iletişim bilgisi güncellemeleri dakikalar içinde yapılabilir.",
  },
];
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 left-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-navy-800 text-xl font-semibold text-gold-400 shadow-lg shadow-black/20 ring-1 ring-white/10 transition-transform hover:scale-105"
      >
        ?
      </button>
      ...
      {open && (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center bg-navy-950/70 p-4"
          onClick={() => setOpen(false)}
        >
          <div
            role="dialog"
            aria-modal="true"
            className="relative w-full max-w-md rounded-md bg-white p-8 shadow-2xl"
          ...
          >
            <button
              ...
              onClick={() => setOpen(false)}
              aria-label="Kapat"
              className="absolute right-4 top-4 text-xl leading-none text-ink-400 transition-colors hover:text-navy-900"
            >
              ×
            </button>

            <p className="eyebrow">Fark Yaratan Bir Yatırım</p>
            <h2 className="mt-3 font-serif text-2xl font-semibold leading-snug text-navy-900">
              Neden profesyonel bir hukuk bürosu sitesi?
            </h2>

            <ul className="mt-6 space-y-3">
              {reasons.map((reason) => (
                <li key={reason.text} className="flex items-start gap-3 text-sm text-ink-600">
            ...
            <a
              ...
              className="mt-7 flex w-full items-center justify-center gap-2 rounded-sm bg-[#25D366] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#1ebd5a]"
            >
